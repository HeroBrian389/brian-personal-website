...
<article class="notion-page">
  ...
  <header class="mb-20 {showProgress ? 'pt-16' : ''}">
    <h1 class="text-4xl md:text-5xl font-extralight mb-8">{page.title}</h1>
    
    {#if showMetadata}
      <div class="text-sm uppercase tracking-[0.2em] text-muted-foreground font-light">
        <p>{formatDate(page.metadata.lastEditedTime)}</p>
      </div>
    {/if}
  </header>
  
  <div class="prose prose-lg dark:prose-invert max-w-none prose-headings:font-normal prose-p:font-light prose-strong:font-normal">
    {#each page.enhancedBlocks as block}
      <Block {block} {enableTypography} />
    {/each}
  </div>
  
  <footer class="mt-20 pt-8 border-t border-foreground/10">
    <div class="text-sm uppercase tracking-[0.2em] text-muted-foreground font-light">
