...
async function fetchQuotesFromNotion(): Promise<Quote[]> {
  try {
    ...
    Object.entries(recordMap.block).forEach(([id, blockWrapper]: [string, any]) => {
      ...
      if (block && (block.type === 'quote' || block.type === 'text' || block.type === 'bulleted_list')) {
        if (block.properties?.title) {
          ...
          } else if (plainText.length > 20 && !plainText.toLowerCase().includes('here are some quotes')) {
...
function extractTags(text: string): string[] {
  ...
  if (lowerText.includes('time') || lowerText.includes('future') || lowerText.includes('past')) themes.push('time');
