...
export function parsePage(recordMap: any): ParsedPage | null {
  ...
  const title = properties?.title ? 
    parseRichText(properties.title).map(t => t.text).join('') : 
    'Untitled';
...
export function enhanceBlock(block: ParsedBlock): EnhancedParsedBlock {
  ...
  const semantics = {
    ...
    headingLevel: block.type.includes('sub_sub_') ? 3 : 
                  block.type.includes('sub_') ? 2 : 
                  block.type.includes('header') ? 1 : undefined
  };
