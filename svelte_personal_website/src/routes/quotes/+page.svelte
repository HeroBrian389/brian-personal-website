<script lang="ts">
  ...
  async function getNewQuote() {
    ...
    if (isTransitioning || quotes.length === 0) {
      console.log('[QUOTES] Skipping - isTransitioning:', isTransitioning, 'quotes.length:', quotes.length);
      return;
    }
...
<div class="min-h-screen bg-background overflow-hidden">
  <div class="container mx-auto px-4 max-w-6xl">
    ...
    <div class="relative h-[calc(100vh-5rem)] flex items-center justify-center">
      <div class="relative w-full max-w-4xl mx-auto px-8">
        <!-- Current quote -->
        {#if currentQuote}
          <div class="absolute inset-0 flex flex-col items-center justify-center" style="opacity: {currentOpacity}; transition: opacity {currentOpacity === 0 ? '1000ms' : '2000ms'} ease-in-out">
            <!-- Quote text -->
            <div class="w-full">
              <p class="text-xl md:text-2xl lg:text-3xl font-extralight leading-tight text-center tracking-tight">
                {currentQuote.text}
              </p>
            </div>
        ...
        {#if nextQuote}
          <div class="absolute inset-0 flex flex-col items-center justify-center" style="opacity: {nextOpacity}; transition: opacity 2000ms ease-in-out">
            <!-- Quote text -->
            <div class="w-full">
              <p class="text-xl md:text-2xl lg:text-3xl font-extralight leading-tight text-center tracking-tight">
                {nextQuote.text}
      ...
      <div class="absolute bottom-16 left-1/2 -translate-x-1/2">
        <button
          ...
          class="group relative px-8 py-3 text-sm uppercase tracking-[0.2em] font-light
                 border border-foreground/20 hover:border-foreground/40 
                 transition-all duration-500 disabled:opacity-50 disabled:hover:border-foreground/20"
        >
          <span class="relative z-10">Next</span>
          <div class="absolute inset-0 bg-foreground/5 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 py-20">
      {#each quotes as quote, i}
        <button
          ...
          class="group text-left p-8 border border-transparent hover:border-foreground/10 
                 transition-all duration-500 relative overflow-hidden"
          in:fade={{ duration: 400, delay: Math.min(i * 50, 200) }}
        >
          <!-- Hover background -->
          <div class="absolute inset-0 bg-gradient-to-br from-foreground/[0.02] to-transparent 
                      opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          
          <!-- Content -->
          <div class="relative">
            <p class="text-sm leading-relaxed mb-4 text-foreground/70 group-hover:text-foreground/90 transition-colors duration-300">
              {quote.text}
            </p>
            {#if quote.author}
              <p class="text-xs uppercase tracking-wider text-muted-foreground/60 group-hover:text-muted-foreground transition-colors duration-300">
                {quote.author}
              </p>
            {/if}
          </div>
          ...
          <!-- Subtle indicator -->
          <div class="absolute bottom-0 left-0 w-full h-px bg-foreground/10 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
        </button>
      {/each}
    </div>
