<script lang="ts">
	...
	const gardenPath: PonderVideo[] = [
		...
		},
		{
			...
			duration: "10:40",
			transition:
				"Pause. Think deeply. Are you living in a way that reaches up? Continue when ready"
		}
	];
...
<div class="bg-background relative">
	...
	<!-- Garden Entry -->
	<header class="flex h-screen items-center justify-center px-4 text-center">
		<div>
			<h1
				class="mb-8 text-5xl font-extralight md:text-6xl"
				in:fade={{ duration: 2000, delay: 100 }}
			>
				Ponder
			</h1>
			<p
				class="text-muted-foreground mb-16 text-sm font-light tracking-[0.2em] uppercase"
			...
			<div class="mb-16 flex justify-center" in:fade={{ duration: 2000, delay: 500 }}>
				<div
					class="bg-foreground/20 h-px transition-all duration-1000"
			...
			<button
				...
				class="text-muted-foreground/60 hover:text-foreground/80 text-sm font-light tracking-[0.2em] uppercase transition-all duration-1000"
	...
	<div class="relative">
		{#each gardenPath as video, index}
			...
			{#if index > 0}
				<div class="flex h-[calc(100vh-5rem)] items-center justify-center px-4 text-center">
					<p
						class="text-muted-foreground/40 text-sm font-light tracking-[0.3em] uppercase"
					>
						{video.transition || "• • •"}
					</p>
				</div>
			...
			<section
				id="clearing-{index}"
				class="garden-clearing flex min-h-screen items-center py-20"
			>
				<div class="container mx-auto max-w-4xl px-4">
					<!-- Video Container -->
					<div class="mb-16">
						<div
							class="relative aspect-video overflow-hidden rounded-sm bg-black/5 backdrop-blur-sm"
						>
					...
					<div class="text-center">
						<h2 class="mb-4 text-2xl font-extralight">{video.title}</h2>

						<p
							class="text-muted-foreground/60 mb-8 text-sm font-light tracking-[0.2em] uppercase"
						>
							{video.author}
						</p>
						...
						{#if index < gardenPath.length - 1}
							<button
								...
								class="text-muted-foreground/40 hover:text-foreground/60 mt-16 text-sm font-light tracking-[0.2em] uppercase transition-all duration-1000"
							>
								Continue ↓
							</button>
						{/if}
		...
		<div class="flex h-screen items-center justify-center px-4 text-center">
			<div>
				<p
					class="text-muted-foreground/40 mb-8 text-sm font-light tracking-[0.3em] uppercase"
				>
					End of the path
				</p>
				<button
...
<style>
	...
	* {
	...
	.energy-orb-1-halo {
		width: 600px;
		height: 600px;
		background: radial-gradient(circle at center, rgba(96, 165, 250, 0.02) 0%, transparent 60%);
